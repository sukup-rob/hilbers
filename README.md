...
│ l │   LEFT │
├───┼────────┤
│ s │   STOP │ <-- `STOP` should not appear in `output.txt` - it is a legacy value.
└───┴────────┘
...
│ l │  DOLEVA │
├───┼─────────┤
│ s │    STOP │ <-- STOP by se v `output.txt` nemělo objevit, ale je součástí kódu, takže ho nechávám přiložený.
└───┴─────────┘
